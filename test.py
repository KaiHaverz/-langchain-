...
class Chat:
    def __init__(self, history=None, base_dir=None):
        # 初始化
        self.chat_model = ChatZhipuAI() # 模型
        self.text_splitter = RecursiveCharacterTextSplitter() # 分词
        self.history = history if history is not None else [] # 历史记录数组
    ...
    def initialize_agent_tools(self):
        if self.base_dir.lower() != "pass":
            ...
            retrieval_tool_instance = self.create_tool("retrieval_tool",
                                                       "This tool handles web page and local document retrieval.",
                                                       retrieval_chain)
    ...
    # 处理第一条消息
    def isFirst(self, base_dir, history_array, question):
        self.base_dir = base_dir
        self.load_history_from_array(history_array)
        self.initialize_agent_tools()
